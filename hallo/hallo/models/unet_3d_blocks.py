...
class CrossAttnUpBlock3D(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        for layer_idx, (resnet, attn, audio_module, motion_module) in enumerate(
            zip(self.resnets, self.attentions, self.audio_modules, self.motion_modules)
        ):
...
#
                ...
                def create_custom_forward(
                    ...
                    def custom_forward(*inputs):
                        ...
                        if save_cross_attention is not None:
                            kwargs["save_cross_attention"] = save_cross_attention
                        if return_cross_attention is not None:
                            kwargs["return_cross_attention"] = return_cross_attention
                        if timestep_now is not None:
                            kwargs["timestep_now"] = timestep_now
                        if is_final_block is not None:
                            kwargs["is_final_block"] = is_final_block
                ...
                if audio_module is not None:
                    # audio_embedding = audio_embedding
                    if return_cross_attention and (timestep == 24) and is_final_block and layer_idx == 2:
                        output = torch.utils.checkpoint.checkpoint(
                            create_custom_forward(
                                audio_module,
                                return_dict=False,
                                return_cross_attention=return_cross_attention,
                                timestep_now=timestep,
                                is_final_block=is_final_block,
                                layer_idx=layer_idx,
                            ),
                            hidden_states,
                        ...
                        if len(output) == 3:
                            hidden_states, _, attn_feat = output
                        elif len(output) == 2:
                            hidden_states, attn_feat = output
                        else:
                            hidden_states = output
                            attn_feat = None
                        hidden_states = hidden_states.unsqueeze(0).permute(0, 2, 1, 3, 4)
