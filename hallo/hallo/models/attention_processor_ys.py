...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        save_cross_attention: bool = False,
    ):
        super().__init__()
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, save_cross_attention=False, **cross_attention_kwargs, ):
        ...
        return self.processor(
            ...
            **cross_attention_kwargs,
            save_cross_attention=save_cross_attention,
        )
...
class AttnProcessor:
    ...
    def __call__(
        ...
        if save_cross_attention:
            ...
            # TODO(Patrick): save cross attention weights
...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        query = attn.to_q(hidden_states, scale=scale) #torch.Size([32, 4096, 320])
        key = attn.to_k(encoder_hidden_states, scale=scale) #torch.Size([32, 4096, 320])
        value = attn.to_v(encoder_hidden_states, scale=scale) #torch.Size([32, 4096, 320])

        ######################### === Save attention map === #########################
        if save_cross_attention:
            # [B*H, Q, D]
            _query = attn.head_to_batch_dim(query)
            _key = attn.head_to_batch_dim(key)

            attention_probs = attn.get_attention_scores(_query, _key, attention_mask)
            attn_map = attention_probs.view(batch_size, attn.heads, attention_probs.shape[1], attention_probs.shape[2]).detach().cpu()

        inner_dim = key.shape[-1]
        head_dim = inner_dim // attn.heads #320 // 8 = 40

        query = query.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2) #torch.Size([32, 8, 4096, 40])
        key = key.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2) #torch.Size([32, 8, 4096, 40])
        value = value.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2) #torch.Size([32, 8, 4096, 40])
        ...
        if save_cross_attention:
            return hidden_states, attn_map  # [B, H, Q_len, K_len]
        else:
            return hidden_states
