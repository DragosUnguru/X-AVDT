...
class AudioTemporalBasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.return_cross_attention = return_cross_attention,
        self.timestep_now = timestep_now,
    ...
    def forward(
        ...
        if self.attn2 is not None:
            ...
            full_attn = full_hidden_states
            bz, sz, c = full_hidden_states.shape
            sz_sqrt = int(sz ** 0.5)
            full_hidden_states = full_hidden_states.reshape(bz, sz_sqrt, sz_sqrt, c).permute(0, 3, 1, 2)
            full_hidden_states = self.zero_conv_full(full_hidden_states).permute(0, 2, 3, 1).reshape(bz, -1, c)

            face_hidden_state = ( #exp
                self.attn2_1(
                    norm_hidden_states,
                    encoder_hidden_states=encoder_hidden_states,
                    attention_mask=attention_mask,
            ...
            face_attn = face_hidden_state
            face_hidden_state = face_hidden_state.reshape(bz, sz_sqrt, sz_sqrt, c).permute(0, 3, 1, 2)
            ...
            lip_attn = lip_hidden_state
            lip_hidden_state = lip_hidden_state.reshape(bz, sz_sqrt, sz_sqrt, c).permute(0, 3, 1, 2)
            lip_hidden_state = self.zero_conv_lip(lip_hidden_state).permute(0, 2, 3, 1).reshape(bz, -1, c)
            
            #attn_feat = (full_hidden_states + face_hidden_state + lip_hidden_state)
            attn_feat = (full_attn + face_attn + lip_attn)
        ...
        if return_cross_attention and timestep_now == 24 and self.unet_block_name == "up" and is_final_block and layer_idx == 2:
            return hidden_states, attn_feat
        else:
            return hidden_states
