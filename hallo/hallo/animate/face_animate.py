...
class FaceAnimatePipeline(DiffusionPipeline):
    ...
    def decode_latents(self, latents, output_type="numpy"):
        ...
        # Convert each frame to a PIL image and return as a list of PIL images
        elif output_type == "pil":
            pil_images = []
            for i in range(video.shape[2]):  # Iterate over frames
                frame = video[0, :, i, :, :].permute(1, 2, 0).cpu().numpy()  # Convert from [C, H, W] to [H, W, C]
                frame = (frame * 255).astype(np.uint8)  # Convert to uint8 for PIL
                pil_images.append(Image.fromarray(frame))  # Convert NumPy array to PIL image
        ...
        **kwargs,
        ...
        if inversion:
            ...
            if src_latents_frames.shape[-1] != latent_size:
                ...
                src_frames_tensor = src_latents_frames.permute(0,2,1,3,4) # (b c f h w) -> (b f c h w)
                src_frames_tensor = rearrange(src_frames_tensor, "b f c h w -> (b f) c h w")
                src_frames_tensor = self.ref_image_processor.preprocess(src_frames_tensor, height=height, width=width)  # (bs, c, width, height)
                src_frames_tensor = src_frames_tensor.to(dtype=self.vae.dtype, device=self.vae.device)
                latents = self.vae.encode(src_frames_tensor).latent_dist.mean # -> (16, 4, 64, 64)
        ...
        ref_image_latents = ref_image_latents * 0.18215  # (b, 4, h, w) torch.Size([3, 4, 32, 32])
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            latent_list = []
            attn_feat_list = []
            for i, t in enumerate(timesteps): # inferece: timesteps[0]=999 -> timesteps[-1]=24, inversion: timesteps[-1]=999 -> timesteps[0]=24
                ...
                if i == 0 and not no_image:
                    if bank_feature:
                        ...
                        #reference_control_reader.update(reference_control_writer, rand_ref=rand_ref)
                ...
                if inversion and return_cross_attention and i==0:
                    #noise_pred, cross_attention_map = self.denoising_unet(
                    output = self.denoising_unet(
                        latent_model_input,
                        t,
                        encoder_hidden_states=encoder_hidden_states,
                ...
                if self.use_guidance:
                    if is_first_frame:
                        self.first_frame_cache.append(
                            latents[:,:,0:1].clone() # get first frame 
                            # latents.clone().mean(2).unsqueeze(2) # get mean 
                        )
                    else:
                        if i <= half_steps:
                            if True:
                                ## inspired from: Sketch-Guided Text-to-Image Diffusion Models
                                src = latents.float().requires_grad_(True)
                                anc = self.first_frame_cache[i].float().requires_grad_(True).repeat(
                                    1, 1, src.shape[2], 1, 1
                                )
                                with torch.enable_grad():
                                    loss = F.mse_loss(anc, src, reduction="mean") # -> should be a classifier
                                ...
                                cond_grad = latents - self.first_frame_cache[i] # cfg
                                cond_grad = cond_grad.to(
                                    device=self.denoising_unet.device, 
                                    dtype=self.denoising_unet.dtype
                                )
                                latents = self.first_frame_cache[i] + self.beta * cond_grad
